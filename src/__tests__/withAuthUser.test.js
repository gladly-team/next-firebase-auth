...
describe('withAuthUser: rendering/redirecting', () => {
  ...
  it('redirects to user-specified destination on the client side when onRedirect is set in the config for unauthorized user', () => {
    expect.assertions(1)
    const withAuthUser = require('src/withAuthUser').default
    const MockSerializedAuthUser = undefined // no server-side user
    useFirebaseUser.mockReturnValue({
    ...
    const MockCompWithUser = withAuthUser({
      whenAuthed: AuthAction.REDIRECT,
    })(MockComponent)
    render(
      <MockCompWithUser
        serializedAuthUser={MockSerializedAuthUser}
  ...
  it('logs a debugging message when using custom redirect', () => {
    expect.assertions(1)
    const withAuthUser = require('src/withAuthUser').default
    const MockSerializedAuthUser = undefined // no server-side user
    useFirebaseUser.mockReturnValue({
    ...
    expect(logDebug).toHaveBeenCalledWith(
      'Redirecting to user-specified endpoint'
    )
  })

  it('throws if onRedirect is not set', () => {
    expect.assertions(1)
    const withAuthUser = require('src/withAuthUser').default
    const MockSerializedAuthUser = undefined // no server-side user
    useFirebaseUser.mockReturnValue({
  ...
  it('throws if auth state "whenAuthed" on onRedirect is not set', () => {
    expect.assertions(1)
    const withAuthUser = require('src/withAuthUser').default
    const MockSerializedAuthUser = undefined // no server-side user
    useFirebaseUser.mockReturnValue({
  ...
  it('throws if auth state onRedirect.whenAuthed.destination is set to an empty string', () => {
    expect.assertions(1)
    const withAuthUser = require('src/withAuthUser').default
    const MockSerializedAuthUser = undefined // no server-side user
    useFirebaseUser.mockReturnValue({
