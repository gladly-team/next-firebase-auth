...
describe('config: server side', () => {
  ...
  it('throws if the user does not provide cookies.name', () => {
    expect.assertions(1)
    const { setConfig } = require('src/config')
  ...
  it('throws if config.firebaseAuthEmulatorHost is set, but not the FIREBASE_AUTH_EMULATOR_HOST env var', () => {
    expect.assertions(1)
    const { setConfig } = require('src/config')
    const mockConfigDefault = createMockConfig()
    const mockConfig = {
  ...
  it('throws if the FIREBASE_AUTH_EMULATOR_HOST env var differs from the one set in the config', () => {
    expect.assertions(1)
    const { setConfig } = require('src/config')
    const mockConfigDefault = createMockConfig()
    const mockConfig = {
  ...
  it('should not throw if the FIREBASE_AUTH_EMULATOR_HOST env variable is set and matches the one set in the config', () => {
    expect.assertions(1)
    const { setConfig } = require('src/config')
    const mockConfigDefault = createMockConfig()
    const mockConfig = {
  ...
  it('does not throw an error if the firebaseAdminInitConfig is not set', () => {
    expect.assertions(1)
    const { setConfig } = require('src/config')
    const mockConfig = {
      ...createMockConfig(),
