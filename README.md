...
* `keys`: An array of strings that will be used to sign cookies; for instance `['xD$WVv3qrP3ywY', '2x6#msoUeNhVHr']`. The `keys` array is passed to the [Keygrip](https://www.npmjs.com/package/keygrip) constructor as described in [the `cookies` package](https://github.com/pillarjs/cookies#cookies--new-cookies-request-response--options--)). **Required** unless `signed` is set to `false`.
