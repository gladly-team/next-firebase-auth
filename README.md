...
| `whenAuthedBeforeRedirect` | The action to take if the user is authenticated and `whenAuthed` is set to `AuthAction.REDIRECT_TO_APP`. One of: `AuthAction.RENDER` or `AuthAction.SHOW_LOADER` or `AuthAction.RETURN_NULL`.        | `AuthAction.RETURN_NULL` |
